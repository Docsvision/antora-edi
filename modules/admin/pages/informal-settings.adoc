...
----
+
.. Добавьте обработчики для новых команд. Обратитесь в xref:system:ROOT:technical-support.adoc[техническую поддержку], чтобы узнать подробности.
// +
...
. Добавьте ЭУ xref:6.1@webclient:layouts:ctrl/table/dataGridControl.adoc["Контрол табличных данных"] в разметку неформализованного документа.
. В свойствах `*Название*` и `*Название плагина*` укажите `EDIMessagesLog`.
. Добавьте скрипт `edi_loadMessageLogSignatureStatus` для события `После загрузки таблицы` для элемента управления xref:6.1@webclient:layouts:ctrl/table/dataGridControl.adoc["Контрол табличных данных"].
