...
. Добавьте скрипт `edi_loadMessageLogSignatureStatus` для события `После открытия карточки` для элемента *root*, чтобы загрузка статуса подписей и подписантов выполнялась асинхронно.
