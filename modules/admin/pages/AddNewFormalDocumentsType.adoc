...
Для работы указанных методов к скрипту карточки должны подключены сборки `{dv}.Edi.DocumentScript.dll` и `{dv}.DocumentsManagement.dll` (располагаются в каталоге клиента {dv}). В объявление пространств имен нужно добавить строки:
